class Human: #ini adalah kelas kakek dari bapaknya yaitu hero, dan cucunya yaitu warrior, support, assasin
    def __init__(self, name, position, speed=1): #Constructor, langsung dijalankan tanpa di panggil
        self.name = name # atribut
        self.__position = position #atribut
        self._speed = speed #atribut
        
    def Movement(self, arah): #ini method
        if arah == "R":
            self.__position += self._speed
        elif arah == "L":
            self.__position -= self._speed;
            
    def getPosition(self): #ini method
        return self.__position
    def setPosition(self, position):
        self.__position = position
        
    def getSpeed(self):
    #  get mengambil nilainya agar bisa di akses
        return self._speed
...
class Warrior(Hero):
    ...
    def special(self, target):
        ...
        self._power = 32
        target._health -= self._power
        print(f"{self.name} menggunakan special skill!")
...
assassin.Movement("R")
print("posisi assassin sekarang adalah:", assassin.getPosition())