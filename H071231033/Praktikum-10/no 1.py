...
def Email():
    ...
    pattern_email = re.fullmatch(r"^(?=.*[\d])[a-z]+(?:\d{1,})?@student.unhas.ac.id$", email)
    if pattern_email:
        return email
    else:
...
def Password():
    ...
    pattern_password = re.fullmatch(r"^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)[A-Za-z\d]{8,20}$", password)
    if pattern_password:
        return password
    ...
    match pilihan:
        ...
        case 4:
            if detail_akun.nama is None and detail_akun.email is None and detail_akun.password is None:
                ...
                detail_akun = Detail()
        case 5:
            if detail_akun.nama != None:
                print("Detail anda sudah ada sebelumnya apa anda yakin ingin mengganti dengan data baru?")
                lanjut = input("Lanjut/Tidak ? = ")
                if lanjut == "Lanjut":
                    pass
                elif lanjut == "Tidak":
                    continue
                else:
                    print("Inputan salah! Keluar dari Program")
                    break
            print(f"{'='*50}")
            nama = Nama()
            email = Email()
